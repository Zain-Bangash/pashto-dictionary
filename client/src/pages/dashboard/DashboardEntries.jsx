...
export default function DashboardEntries() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-display text-warm mb-6">All Entries</h1>
      ...
      {entries.length === 0 ? (
        ...
        <ul className="space-y-3">
          {entries.map((entry) => {
            const s = STATUS_COLORS[entry.status] ?? STATUS_COLORS.pending;
            return (
              <li key={entry._id} className="bg-white/[0.035] border border-white/[0.08] rounded-[20px] p-4 flex items-center justify-between gap-3">
                <div className="flex flex-col gap-0.5 overflow-hidden">
                  <div dir="rtl" className="font-pashto text-warm text-xl" style={{ lineHeight: 1.7 }}>
                    {entry.pashto}
                  </div>
                  <p className="text-sm font-ui text-muted truncate">{entry.definitions?.[0]?.text}</p>
                </div>
                <span
