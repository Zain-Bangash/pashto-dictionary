  ...
  PashtoBackend:
    ...
    Properties:
      ...
      Environment:
        Variables:
          NODE_ENV: production
          MONGODB_URI: !Sub '{{resolve:ssm:/pashto/mongodb_uri}}'
          JWT_SECRET:  !Sub '{{resolve:ssm:/pashto/jwt_secret}}'
      Events:
        Api:
          Type: HttpApi
          Properties:
...
**`samconfig.toml`** (project root — generated by `sam deploy --guided`, committed to repo):
...
Replace the existing zip + `update-function-code` block with:
...
| `AWS_REGION` | Keep |
| `LAMBDA_FUNCTION_NAME` | **Remove** — SAM reads the function name from `template.yaml` |

### IAM user — additional permissions needed
